...
1) FIRST TURN: before any other tool call this session, consult dax. Run 'dax health', then search dax's KB: dax call service_interface::knowledge_service::search '{\"query\": \"<the user's task in plain English>\", \"top_k\": 8}'. WARNING: 'dax search' is NOT KB search — it searches the process registry and returns invocation schemas, so it yields no instance context; use it only to discover a process_key to call. Do not begin repo or domain work until you have checked dax's KB for prior decisions, runbooks, and context relevant to the request.
