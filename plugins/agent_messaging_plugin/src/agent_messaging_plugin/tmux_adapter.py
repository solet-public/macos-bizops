...
def _effective_spawn_env(spec: Mapping[str, object]) -> Mapping[str, str]:
    ...
    provider, and not the daemon's own inherited environment alone.

    This process's own environment (``os.environ``), overlaid with
    ``spec["provider_env"]`` when present -- the same vault-resolved
    per-spawn inference-provider overlay :func:`_env_pairs` threads into the
    tmux server via explicit ``-e`` pairs (parity with
    ``headless_adapter._apply_provider_env``). The overlay is the seam by
    which a spawn's provider differs from the daemon's own.

    ⚠ One honest limit, not silently papered over: when a tmux SERVER
    already exists, a new pane inherits that server's environment, which is
    the environment of whoever started the server -- not necessarily this
    process's. In the deployed shape (launchd daemon creates the server on
    first spawn) the two are the same; under an operator-started
    pre-existing server they can diverge.
    """
