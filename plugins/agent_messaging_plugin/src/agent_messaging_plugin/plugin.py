...
def _spawn_dispatch_overrides_from_params(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    model, effort, allowed_tools, permission_mode, provider,
    allow_askuserquestion) — split out of ``_spawn_session_request_from_params``
    to keep it under the radon cc threshold; these fields share one purpose
    (per-spawn dispatch overrides), so grouping them is a real seam, not an
    arbitrary split."""
    return {
