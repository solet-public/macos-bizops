...
Mirrors comfyui_image_generation_plugin's completion_templates.py shape (the
doctrine's traced worked example) — same job_metadata/completion_handlers
structure, routed through the same
``service_interface::inference_service::process_results`` /
``process_error`` process keys. These verbs return raw structured data (schema
names, table lists, column shapes, TSV handles) rather than a generated
...
def _instructions(action_name: str, outcome: Literal["result", "error"]) -> list[str]:
    if outcome == "result":
        return [
            f"An external_postgres_plugin '{action_name}' job completed successfully.",
    ...
    return [
        f"An external_postgres_plugin '{action_name}' job ended with an error.",
        "The error message in job_error is already topology-safe (no host, user, or "
        "password detail) and is safe to relay to the user verbatim.",
        "Reply with action using the active IO plugin's post_message "
        "(process_key: plugin::<io_plugin>::post_message) if the flow calls for messaging "
        "the user directly.",
    ]
