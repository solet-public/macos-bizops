"""Single-source homunculus-child spawn mechanics for the two spawn paths.

Two callers spawn a homunculus child process and they MUST agree on the launch
...
the core runtime-dir resolver. The supervisor (the process keeping the homunculus
...
# The entrypoint every homunculus child is launched as. Module form (``-m``) so
...
def build_child_command(interpreter: str, app_home: Path) -> list[str]:
    """The ``ProgramArguments``-equivalent for a spawned homunculus child."""
    return [interpreter, "-m", _ANANTA_CLI_MODULE, _APP_HOME_FLAG, str(app_home)]


def spawn_homunculus_child(
    *,
    interpreter: str,
    app_home: Path,
    homunculus_name: str,
    ...
    """Spawn one homunculus child and return its live :class:`subprocess.Popen`.
    ...
    Args:
        ...
        homunculus_name: Propagated as ``HOMUNCULUS_NAME`` and used to
        ...
        extra_env: Caller-specific env on top of the inherited environment
            + ``HOMUNCULUS_NAME`` (e.g. the swap's explicit
...
__all__ = ["build_child_command", "spawn_homunculus_child"]
