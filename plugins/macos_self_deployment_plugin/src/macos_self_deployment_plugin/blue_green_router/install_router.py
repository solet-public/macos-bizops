...
Usage:
    .venv/bin/python3 plugins/macos_self_deployment_plugin/src/macos_self_deployment_plugin/blue_green_router/install_router.py <homunculus_name> \\
        [--public-port <PORT>]
...
only as an operator-debug override. This lets multiple homunculi on the
...
* ``<name>.router.port`` — read by port_manager.py to skip the router's
  port from homunculus allocation bands.
* ``<name>.bridge.port`` — read by the MCP bridge subprocess to discover
  ...
  the canonical writer of this file; previously homunculus children wrote it
  per-color and the canonical drifted).

Path overrides (smoke harness only):
    --plist-path <PATH>      Override default ~/Library/LaunchAgents/local.homunculus.<name>.router.plist
    --unit-path <PATH>       Override default ~/.config/systemd/user/local.homunculus.<name>.router.service
...
# Dynamic router port range. Lives ABOVE homunculus allocation bands
# (port_manager.py: blue 8101-8149, green 8150-8198, probe 8200-8299) so
# the router never collides with the homunculus — and so multiple homunculi on the
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        _write_router_port_files(args.homunculus_name, args.public_port)
    except InstallError as exc:
        print(f"install_router: {exc}", file=sys.stderr)
        return 1
    print(
        f"install_router: OK — router for homunculus={args.homunculus_name!r} "
...
def _find_free_router_port() -> int:
    ...
    127.0.0.1 so multi-homunculus installs each pick distinct ports.
    """
...
def _write_router_port_files(homunculus_name: str, port: int) -> tuple[Path, Path]:
    """Write the router's chosen public port to both discovery files.

    Two files in ``~/.ananta/runtime/`` receive the same port value:

    * ``<name>.router.port`` — read by ``port_manager.py`` so this homunculus's
    ...
    * ``<name>.bridge.port`` — read by the MCP bridge subprocess via
      ``read_port_file(service_name='bridge', homunculus_name=<name>)``
      to discover the router as the single ingress for the homunculus.

    Slice 1 of the bridge-port-routing-and-session-lifecycle design makes
    the router the canonical writer of ``<name>.bridge.port``; homunculus children
    are decoupled from canonical-file ownership (per-color sibling files
    continue to be written by homunculus children until Slice 3 retires them).
    """
    runtime_dir = Path.home() / ".ananta" / "runtime"
    runtime_dir.mkdir(parents=True, mode=0o700, exist_ok=True)
    router_port_file = runtime_dir / f"{homunculus_name}.router.port"
    router_port_file.write_text(str(port))
    router_port_file.chmod(0o600)
    bridge_port_file = runtime_dir / f"{homunculus_name}.bridge.port"
...
def _build_context(args: argparse.Namespace) -> dict[str, str]:
    ...
    return {
        "LAUNCHD_LABEL": launchd_label(args.homunculus_name),
        ...
        "HOMUNCULUS_NAME": args.homunculus_name,
