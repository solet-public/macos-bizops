...
Genesis has no parent homunculus to read per-plugin config baselines from
...
plugin-owned substitute: static config for the `macos-free-homunculus`
...
that the profile is wrong." Per-homunculus / per-installation fields are
...
## Per-homunculus fields the materialize step (Slice D) MUST inject

| File | Field(s) | Injected value |
|---|---|---|
| `postgres_state_management_plugin.json` | `schema` | the homunculus name (mirrors `write_postgres_config`) |
...
reference environment. Nothing per-homunculus to inject. GENESIS
...
Not every plugin in `macos-free-homunculus.yaml`'s allowlist needs a
...
only static connection defaults; genesis injects per-homunculus database,
