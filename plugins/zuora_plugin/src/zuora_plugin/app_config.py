"""Zuora tenant config loaded from the address book.
...
class AppConfigError(RuntimeError):
    """Raised when the ``zuora_tenant`` address-book entry cannot supply config."""
...
class ZuoraTenantConfig:
    """Resolved Zuora tenant identity + credentials.
    ...
    def __repr__(self) -> str:
        return (
            f"ZuoraTenantConfig(base_url={self.base_url!r}, "
            f"client_id={self.client_id!r}, client_secret={_SECRET_REDACTION!r})"
        )


class AppConfigLoader:
    """Resolve the Zuora tenant config from ``service_interface::address_book_service``.
    ...
    def load(self) -> ZuoraTenantConfig:
        """Return the resolved Zuora tenant config (vault reference swapped)."""
    ...
    def _build_config(self, entries: list[dict[str, Any]]) -> ZuoraTenantConfig:
...
def _missing_entry_message() -> str:
    ...
    return (
        ...
        "Register it before running any zuora verb. Example:\n"
