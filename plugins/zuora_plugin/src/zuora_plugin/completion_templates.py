...
Mirrors salesforce_plugin's completion_templates.py shape (itself mirroring
comfyui_image_generation_plugin, the doctrine's traced worked example) — same
job_metadata/completion_handlers structure, routed through
``service_interface::inference_service::process_results`` /
``process_error``. These verbs return raw structured data (TSV handles,
object fields, write confirmations) rather than a generated image, so the
...
def _instructions(action_name: str, outcome: Literal["result", "error"]) -> list[str]:
    if outcome == "result":
        return [
            f"A zuora_plugin '{action_name}' job completed successfully.",
            "The raw result is available as job_payload — relay it to the user (or use it "
            "to continue your reasoning) as returned; technical fields like path, row_count, "
            "columns, total_size, truncated, or object id are meaningful to the caller and "
    ...
    return [
        f"A zuora_plugin '{action_name}' job ended with an error.",
        "The error message in job_error is already topology-safe (no tenant host or "
        "session detail) and is safe to relay to the user verbatim.",
        "Reply with action using the active IO plugin's post_message "
        "(process_key: plugin::<io_plugin>::post_message) if the flow calls for messaging "
        "the user directly.",
    ]
