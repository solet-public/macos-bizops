...
operator ``stop_self`` (which writes it to persist). the homunculus's SIGTERM handler reads
...
def _resolve_name(homunculus_name: str | None) -> str:
    """Resolve the homunculus name, falling back to ``HOMUNCULUS_NAME``.

    Matches the writer's resolution (the plugin always passes the same
    ``self._homunculus_name`` it took from the environment), so the read and
    ...
    """
    name = (homunculus_name or os.environ.get("HOMUNCULUS_NAME", "")).strip()
    if not name:
        raise RuntimeError(
            "cannot resolve drain-sentinel path: no homunculus_name and "
            "HOMUNCULUS_NAME is unset"
        )
    return name


def draining_sentinel_path(homunculus_name: str | None = None) -> Path:
    """Canonical drain-sentinel path for ``homunculus_name``.

    The file is flat in the runtime dir with the name in the FILENAME (not a
    per-name subdir): ``<runtime_dir>/<name>.draining``. ``homunculus_name=None``
    resolves it from ``HOMUNCULUS_NAME``.
    """
    name = _resolve_name(homunculus_name)
    return get_runtime_dir(name) / f"{name}{DRAINING_SENTINEL_SUFFIX}"


def is_draining(homunculus_name: str | None = None) -> bool:
    ...
    """
    try:
        return draining_sentinel_path(homunculus_name).exists()
